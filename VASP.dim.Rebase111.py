...
iFileName = str(input('    Base geometry (def=CENTCAR) : '))
if iFileName == '': iFileName = 'CENTCAR'
...
######## Get file: Atom coordinates
iMODEFileName = str(input('    Reaction vector (def=NEWMODECAR) : '))
if iMODEFileName == '': iMODEFileName = 'NEWMODECAR'

try:
	# Opening
	CodeStatus('Opening reaction vector file ...', end='')
	with open(iMODEFileName, 'r') as f:
		iMODEfile = f.readlines()
		f.close()
	print('Ok')

	# Get reaction vector
	CodeStatus('Retrieving '+str(NAtoms)+' coordinates', end=' ... ')
	RxVect = []
	for iAt in range(NAtoms):
		RxVect.append([float(j) for j in iMODEfile[iAt].split()])
	print('Ok')

	# Test
	# for i in RxVect: print(i)
	CodeStatus('Rx vector magnitude='+FixNum((sum([sum([j ** 2 for j in i]) for i in RxVect]))**.5, 4))
...
# Real Reaction vector
CodeStatus('Adapting Rx vector to real coordinates', end=' ... ')
RealRxVect = []
for iAt in RxVect:
	RealRxVect.append([sum([iAt[j]*RealBase[j][i] for j in range(3)]) for i in range(3)])
print('Ok')
...
# Rx vector to new rectangular base
CodeStatus('Adapting reaction vector to rectangular base', end=' ... ')
DirectRxVector = []
for iRRxV in RealRxVect:
	DirectRxVector.append([iRRxV[i]/MaxPos[i] for i in range(3)])
print('Ok')
# test
# for i in DirectRxVector: print(i)

# Normalization
NormalFactorMag = (sum([sum([j ** 2 for j in i]) for i in DirectRxVector]))**.5
CodeStatus('Normalizing reaction vector: Current magnitude='+FixNum(NormalFactorMag, 4), end=', new= ')
for i in range(len(DirectRxVector)):
	for j in range(3):
		DirectRxVector[i][j] = DirectRxVector[i][j]/NormalFactorMag
print(FixNum(sum([sum([j**2 for j in i]) for i in DirectRxVector]), 6))



# Writting
FileName = str(input(' '*4+'Saving in new file. Filename coordinates (deafult=POSCAR) : '))
...
RxVectFileName = str(input(' '*4+'Saving in new file. Reaction vector (deafult=MODECAR) : '))
if RxVectFileName == '': RxVectFileName='MODECAR'

with open(RxVectFileName, 'w') as f:
	for i in DirectRxVector:
		for j in i:
			f.write(FixStrLead(FixNumExp(j,10), 20))
		f.write('\n')
